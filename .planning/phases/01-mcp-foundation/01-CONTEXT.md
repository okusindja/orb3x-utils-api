...
- **D-04:** Limiter state is in-memory / per-instance (stateless, no Redis). Acceptable that counts reset on cold start / instance recycling — this is the free-tier constraint, not a defect. **Live finding (2026-06-18):** a 65-request burst against the production endpoint returned zero 429s — Vercel Fluid Compute spread the requests across warm instances so no single in-memory counter reached 60. The middleware logic is correct (unit tests 5/5 green) but per-instance state cannot enforce a *global* per-IP cap on horizontally-scaled serverless. Accepted as best-effort friction; a real global limit is deferred to v2 (see REQUIREMENTS.md SEC-01 — Vercel Firewall WAF or a shared KV store).
